...
//$cmd->conexion->query("SET innodb_lock_wait_timeout = 500"); Mala read only
...
if (file_exists('/tmp/bm_tmp_1.csv')) {
    try {
        unlink('/tmp/bm_tmp_1.csv');
    } catch (Exception $e) {
        $cmd->logs->error("[SPLIT] Error al borrar archivo /tmp/bm_tmp_1.csv", $e->getMessage(), 'logs_neutralidad');
    }
}
...
GROUP BY
	...
	bm_tmp_1.`id_item`,
	bm_tmp_1.`id_plan`,
	DATE_FORMAT(from_unixtime(`bm_q`.`clock`),'" . $grp . "') INTO OUTFILE '/tmp/bm_tmp_1.csv'";
...
$INSERT_bm_stat1 = $cmd->conexion->query("LOAD DATA INFILE '/tmp/bm_tmp_1.csv' INTO TABLE `bm_stat1`", false, 'logs_neutralidad');
...
/*
 ...
 IF( bm_statp_temp1.value*bsw_pesos.value/($getPesosSum) IS  NULL ,bm_statp_temp1.value,
bm_statp_temp1.value*bsw_pesos.value/($getPesosSum)) as wvalue,
 IF( bm_statp_temp1.std_samp*bsw_pesos.value/($getPesosSum) IS  NULL ,bm_statp_temp1.std_samp,
bm_statp_temp1.std_samp*bsw_pesos.value/($getPesosSum)) as wstd_samp,
 IF( bm_statp_temp1.std_pop*bsw_pesos.value/($getPesosSum) IS  NULL ,bm_statp_temp1.std_pop,
