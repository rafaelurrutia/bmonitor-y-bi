...
class ServerQoe {
    ...
    ORDER BY `clock`";
		...
		foreach ($getHistoryRESULT as $keyHistory => $hvalue) {
			...
			$reportValue_{$loop}[] = "( " . $hvalue['id_host'] . ", " . $host->groupid . ", " . $param->clockS . " , '" . $hvalue['ip'] . "', '" . $host->city . "', '" . $host->state . "', '" . $host->country . "', '" . $host->isp . "',
                         " . $host->latitude . ", " . $host->longitude . ", FROM_UNIXTIME(" . $hvalue['clock'] . "),
                             '" . trim($hvalue['ServerName']) . "', " . $download . ", " . $upload . ", " . $latency . ", " . $percenDownloadResult . ", " . $percenUploadResult . ", 'Internet Explorer 9.0', 'Windows 7 Home', '','" . $host->idLocation . "')";
		...
		$insertReportSpeedValueSQL = "INSERT INTO `report_speedtest` ( `idHost` , `groupid`, `date`, `ip`, `city`, `region`, `country`, `isp`, `latitude`, `longitude`, 
                                            `testDate`, `serverName`, `download`, `upload`, `latency`,  `pdownload`, `pupload`,
                                                    `browser`, `operatingSystem`, `userAgent`,`idLocation`) VALUES ";
