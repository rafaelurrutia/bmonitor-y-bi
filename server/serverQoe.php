	...
	public function qoeReport($server)
	{
		...
		*/
		foreach ($getPlanPercentil as $key => $value) {
		    ...
		    $insertInform = $this->conexion->query("INSERT INTO `bm_inform` (`clock`, `groupid`, `idPlan`, `idItem`, `format`, `type`, `value`, `location`) VALUES 
			('$param->clock', $groupid, $idPlan, $idItem, 'decimal', '$type', '$valueParam','$location')",false,'logs_qoe');
			
			if (!$insertInform) {
			    $this->logs->error("[QoS] Error al guardar resultado, INSERT INTO `bm_inform` (`clock`, `groupid`, `idPlan`, `idItem`, `format`, `type`, `value`, `location`) VALUES ('$param->clock', $groupid, $idPlan, $idItem, 'decimal', '$type', '$valueParam','$location')", NULL);
			    //return false;
			} 
