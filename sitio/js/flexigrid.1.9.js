...
(function ($) {
	...
	function searchVersion (dataString,versionSearch) {
		var index = dataString.indexOf(versionSearch);
		if (index == -1) return;
		return parseFloat(dataString.substring(index+versionSearch.length+1));
	}

	if ($.browser.msie) {
		var versionSearch = "MSIE";
	}
	
	if ($.browser.chrome) {
		var versionSearch = "Chrome";
	}
	...
	$.browser.version = searchVersion(navigator.userAgent,versionSearch)
			|| searchVersion(navigator.appVersion,versionSearch)
			|| "an unknown version";
	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			rePosDrag: function () {
				...
				$('thead tr:first th:visible', this.hDiv).each(function () {
					...
					$('div:eq(' + n + ')', g.cDrag).css({
						'left': (!(/mozilla/.test(navigator.userAgent.toLowerCase()) && !/webkit/.test(navigator.userAgent.toLowerCase())) ? cdpos - cdcounter : cdpos) + 'px'
					}).show();
					cdleft = cdpos;
					cdcounter++;
				});
			...
			switchCol: function (cdrag, cdrop) { //switch columns
				...
				if (/msie/.test(navigator.userAgent.toLowerCase()) && $.browser.version < 7.0) {
