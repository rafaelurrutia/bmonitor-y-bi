...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
		    idFlexigrid: false,
		    themeroller: true, //default height
			...
			qtype: '',
			nomsg: 'No items',
			titleHideAndShowColums: 'Hide/Show Columns',
		...
		if(language.code == '' || language.code == 'es'){
			p.pagestat = 'Viendo del {from} al {to} de {total} artículos';
			p.procmsg = 'Procesando, por favor espere';
			p.pagetext = 'Página';
			p.outof = 'de';
			p.findtext = 'Buscar';
			p.nomsg = 'Buscar';
			p.nomsg = 'No hay elementos';
			p.titleHideAndShowColums = 'Ocultar / Mostrar Columnas';
			p.goToBeginning = 'Ir al comienzo';
			p.rewind = 'rebobinar';
			p.fastForward = 'avance rápido';
			p.goToEnd = 'ir a la final';
			p.refresh = 'refrescar';
		}
		...
		var g = {
            ...
            hideBody: function () {
                ...
                $('div.ptogtitle span.ui-icon',g.mDiv).toggleClass('ui-icon-circle-triangle-n');
                $('div.ptogtitle span.ui-icon',g.mDiv).toggleClass('ui-icon-circle-triangle-s');
                $('#ptogtitle').trigger('click');
                $('div.ptogtitle span.ui-icon',g.mDiv).trigger('click');
                if(p.statusTable == false) {
                    p.statusTable = true;
                } else {
                    p.statusTable = false;
                }
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
				    if(p.themeroller) {
                        ...
                        if(i == 0) {
                            var classSeparator = "ui-corner-left";
                        } else if (parseInt(i)+parseInt(1) == p.buttons.length) {
                            classSeparator = "ui-corner-right";
                        } else {
                            classSeparator = "";
                        }
                               
                        btnDiv.className = 'fbuttonTheme ui-button ui-widget ui-state-default '+ classSeparator  +' ui-button-text-icon-primary';   
                        
                        
                        
                        btnDiv.innerHTML = ('<span class="ui-button-icon-primary ui-icon"></span>');
                        
                        
                        if (btn.bclass) {
                            
                            if(btn.bclass == 'delete') {
                                btn.bclass = 'ui-icon-trash';
                            } else if (btn.bclass == 'add') {
                                btn.bclass = 'ui-icon-document';
                            }
                            
                            $('span', btnDiv).addClass(btn.bclass);   
                        } 
                        
                        btnDiv.innerHTML += ("<span class='ui-button-text'>") + (btn.hidename ? "&nbsp;" : btn.name) + ("</span>");
					...
					if (btn.onpress) {
						$(btnDiv).click(function () {
						    if(this.option) {
						       this.onpress(this.id || this.name, g.gDiv,this.option); 
						    } else {
			...
			if(p.themeroller) {
                $('.tDiv2 .fbuttonTheme').hover(function () {
		...
		// add pager
		if (p.usepager) {
			g.pDiv.className = 'pDiv fg-toolbar ui-toolbar ui-widget-header ui-corner-bl ui-corner-br ui-helper-clearfix';
			g.pDiv.innerHTML = '<div class="pDiv2"></div>';
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <button id="beginning" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only pFirst pButton" role="button" aria-disabled="false" title="'+p.goToBeginning+'"><span class="ui-button-icon-primary ui-icon ui-icon-seek-start"></span><span class="ui-button-text">'+p.goToBeginning+'</span></button><button id="'+p.rewind+'" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only pPrev pButton" role="button" aria-disabled="false" title="'+p.rewind+'"><span class="ui-button-icon-primary ui-icon ui-icon-seek-prev"></span><span class="ui-button-text">'+p.rewind+'</span></button> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" /> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <button id="forward" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only pNext pButton" role="button" aria-disabled="false" title="'+p.fastForward+'"><span class="ui-button-icon-primary ui-icon ui-icon-seek-next"></span><span class="ui-button-text">'+p.fastForward+'</span></button><button id="end" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only pLast pButton" role="button" aria-disabled="false" title="'+p.goToEnd+'"><span class="ui-button-icon-primary ui-icon ui-icon-seek-end"></span><span class="ui-button-text">'+p.goToEnd+'</span></button> </div> <div class="btnseparator"></div> <div class="pGroup"> <button id="refresh" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only pReload pButton" role="button" aria-disabled="false" title="'+p.refresh+'"><span class="ui-button-icon-primary ui-icon ui-icon-refresh"></span><span class="ui-button-text">'+p.refresh+'</span></button> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		// add title
		if (p.title) {
			g.mDiv.className = 'mDiv fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix';
			g.mDiv.innerHTML = '<div class="ftitle">' + p.title + '</div>';
			$(g.gDiv).prepend(g.mDiv);
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div id="ptogtitle" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only ptogtitle" role="button" aria-disabled="false" title="Minimize/Maximize Table"><span class="ui-button-icon-primary ui-icon ui-icon-circle-triangle-n"></span><span class="ui-button-text">toggle</span></div>');
                ...
                $('div.ptogtitle', g.pDiv).hover(function () {
                    $(this).addClass('ui-state-hover'); 
                }, function () {
                    $(this).removeClass('ui-state-hover');
                });
		...
		if ($('th', g.hDiv).length) {
			...
			$(g.nBtn).addClass('nBtn')
				.html('<div></div>')
				.attr('title', p.titleHideAndShowColums)
				.click(function () {
					$(g.nDiv).toggle();
					return true;
				}
    ...
    $.fn.hideBody = function () { // function to reload grid
        return this.each(function () {
            if (this.grid) this.grid.hideBody();
        });
    }; //end flexToggleCol
    $.fn.showBody = function () { // function togle grid
        return this.each(function () {
            if (this.grid) this.grid.showBody();
        });
    }; //end flexToggleCol
    ...
    //end flexSearch
    $.fn.selectedRows = function(p) {// Returns the selected rows as an array, taken
    	// and adapted from
